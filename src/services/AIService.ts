...
export class AIService {
  ...
  static async processOrder(userMessage: string, userState: string): Promise<any> {
    ...
    if (userState.toUpperCase() === "PIZZA_MENU" || userState.toUpperCase() === "PF_PIZZA_MENU") {
      ...
      - \`pizzas\`: Uma lista de objetos contendo \`sabor\`, \`tamanho\` e \`borda\`(se for meia a meia, \`sabor\` será uma lista com 2 sabores)
      - \`observacoes\`: Qualquer informação extra que o cliente mencionou
      - \`summary\`: Resumo amigável do pedido
      Exemplo de entrada do usuário: "Quero uma pizza grande de calabresa com borda de catupiry, uma broto de portuguesa. Pode caprichar no recheio!". Agora gere o JSON correspondente para a seguinte mensagem do usuário, sem adicionar explicações ou texto extra: "${userMessage}"`;
      ...
      - \`observacoes\`: Qualquer informação extra que o cliente mencionou
      - \`summary\`: Resumo amigável do pedido
      Exemplo de entrada do usuário: "Quero uma fogazza de portuguesa com borda de catupiry. Pode caprichar no recheio!". Agora gere o JSON correspondente para a seguinte mensagem do usuário, sem adicionar explicações ou texto extra: "${userMessage}"`;
    } else {
      prompt = `Interprete a seguinte mensagem do usuário e extraia os dados do pedido. Retorne apenas um JSON estruturado com os seguintes campos:
      - \`pizzas\`: Uma lista de objetos contendo \`sabor\`, \`tamanho\` e \`borda\`(se for meia a meia, \`sabor\` será uma lista com 2 sabores)
      - \`fogazzas\`: Uma lista de objetos contendo \`sabor\` e \`borda\`
      ...
      Exemplo de entrada do usuário: "Quero uma pizza grande de calabresa com borda de catupiry, uma broto de portuguesa e uma fogazza de portuguesa. Também quero duas cocas e uma fanta. Pode caprichar no recheio!". Agora gere o JSON correspondente para a seguinte mensagem do usuário, sem adicionar explicações ou texto extra: "${userMessage}"`;
    }
